...
const madisonMarshall = users.find(user => user.firstName === 'Madison' && user.lastName === 'Marshall');
console.log(madisonMarshall.id);

// 2) Who wrote the first comment (assuming the first comment is in position 0 of the comments array)
const firstCommentUserId = comments[0].userId;
const firstCommentName = users.find(user => user.id === firstCommentUserId)
console.log(firstCommentName.firstName);

// 3) Which user commented 'OK great thanks'?
// Find the comment with the text 'OK great thanks'
const comment = comments.find(comment => comment.text === 'OK great thanks');

// Get the user ID from the found comment
const commentUserId = comment.userId;

// Find the user with the obtained user ID
const commentUser = users.find(user => user.id === commentUserId);

// Output the user's first name
console.log(commentUser.firstName);
